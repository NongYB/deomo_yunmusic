...
Page({
 ...
 async getvideoGroupListData(){
    ...
    this.setData({
      ...
      navId:videoGroupListData.data[0].id    //获取id 默认显示第一个导航标签数据
    })
     //调用获取视频列表数据
 ...
 async getvideoList(navId){
   if(!navId){ //判断navId为空串的情况 上一个函数也是异步函数所以会先执行传来空串（解决异步问题）
     return;
   }
    ...
    }else if(videoListData.datas.length===0){
      wx.showToast({
        title: '暂无推荐视频',
        icon:'none'
      })
 ...
 handleTimeUpdate(event){
   ...
   *  1.如果有，在原视频的播放记录中修改播放时间为当前的播放时间
